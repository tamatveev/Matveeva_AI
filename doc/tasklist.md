...
| 10 | Уведомления при сохранении заявки (Telegram + email) | ✅ Готово | 2026-02-25 |
...
### 10. Уведомления при сохранении заявки (Telegram + email)

- [x] Config: опциональные `TELEGRAM_NOTIFY_CHAT_ID`, `SMTP_*`, `NOTIFY_EMAIL`
- [x] Класс `EmailNotifier` — отправка письма о новой заявке через smtplib
- [x] Handler: после успешной записи заявки — отправка в Telegram (если задан chat_id) и вызов EmailNotifier
- [x] Ошибки отправки уведомлений только логировать, не показывать пользователю
- [x] Обновить `.env.example`, `vision.md`, `tasklist.md`

**Тест:** задать `TELEGRAM_NOTIFY_CHAT_ID` и/или SMTP-переменные, оформить заявку — уведомление приходит в Telegram и/или на почту.
