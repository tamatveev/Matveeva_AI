...
| 8 | Деплой в облако (Railway) | ✅ Готово | 2026-02-25 |
| 9 | Документация проекта (README) | ✅ Готово | 2026-02-25 |
| 10 | Уведомление о заявке в Telegram | ✅ Готово | 2026-02-25 |
| 11 | Согласие на обработку данных и рассылку | ✅ Готово | 2026-03-03 |
...
- [x] Создать проект на Railway, подключить репозиторий
- [x] Задать переменные окружения (в т.ч. `GOOGLE_SERVICE_ACCOUNT_JSON` с содержимым ключа)
- [x] Бот работает на Railway в режиме 24/7

**Тест:** бот доступен в Telegram и отвечает, работая на Railway.

---

### 9. Документация проекта (README)

- [x] Обновить README.md: описание, стек, архитектура
- [x] Добавить Mermaid-диаграммы (архитектура, запуск/деплой)
- [x] Актуальный статус и ссылки на doc/

**Тест:** README даёт полное представление о проекте и способах запуска.

---

### 10. Уведомление о заявке в Telegram

- [x] Config: опциональный `TELEGRAM_NOTIFY_CHAT_ID`
- [x] Handler: после успешной записи заявки — отправка сообщения в заданный чат (если chat_id задан)
- [x] Ошибки отправки уведомления только логировать, не показывать пользователю
- [x] Обновить `.env.example`, `vision.md`, `tasklist.md`

**Тест:** задать `TELEGRAM_NOTIFY_CHAT_ID`, оформить заявку — уведомление приходит в Telegram.

---

### 11. Получение согласия на обработку данных и рассылку рекламных материалов

- [x] Config: две новые переменные — `CONSENT_DATA_PROCESSING_PDF_URL` и `CONSENT_ADVERTISING_PDF_URL` (ссылки на PDF в Google Drive)
- [x] При намерении пользователя оставить заявку — до запроса личных данных отправить оба PDF и запросить согласие (кнопки «Согласен» / «Не согласен» или эквивалент)
- [x] Только после подтверждения согласия переходить к сбору имени, почты и прочих данных заявки; при отказе — не собирать данные и вежливо завершить сценарий
- [x] Обновить `.env.example`, `vision.md`

**Тест:** выбрать «Оставить заявку» — бот присылает два PDF, запрашивает согласие; после «Согласен» запрашивает данные; после «Не согласен» не запрашивает данные.
